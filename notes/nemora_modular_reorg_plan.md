...
2. **Central distributions registry (`nemora.distributions`)**
   ...
   - [x] Expose shared APIs so ingest, fit, sampling, and synthesis can consume consistent definitions.
   - [x] Document extension points for user-supplied distributions.

3. **Distribution fitting (`nemora.fit`) – Alpha delivered**
...
5. **Sampling engine (`nemora.sampling`)**
   ...
   - [ ] Benchmark accuracy and ensure compatibility with `fit` outputs. *(See `notes/sampling_module_plan.md` for detailed roadmap, including BootstrapResult adoption plan.)*
...
9. **Documentation & communication**
   ...
   - [ ] Draft notes highlighting the project’s early stage but rapid iteration plan (fit alpha quickly, other modules phased later).
...
10. **Testing & CI strategy**
    - [ ] Expand unit/integration tests per module; keep coverage gating fit alpha milestone.
...
6. [x] Expose synthesis bootstrap consumption through the CLI/tests once helpers stabilize (e.g., `nemora sampling describe-bootstrap`), then document the workflow in README + notes.
   - [x] Prototype a Typer subcommand that loads a stand table (CSV/Parquet), auto-fits or parses parameters, and renders the metadata via `nemora.synthesis.helpers`.
   - [x] Add pytest coverage for the CLI + helper plumbing (unit + CLI smoke test).
   - [x] Update README + docs/howto/synthesis.md with CLI usage, JSON examples, and troubleshooting notes.
